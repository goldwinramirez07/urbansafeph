...
if (!$dateTime || $dateTime->format($dateTimeFormat) !== $date) {
    ...
    if ($timestamp) {
        ...
        header("Refresh: 1; url=http://urbansafeph.com/User/Member/flood-report1.php");
        exit();
    }
} else {
...
if (isset($_FILES['img']) && $_FILES['img']['error'] == UPLOAD_ERR_OK) {
    ...
    if (in_array($fileType, $allowedTypes)) {
        ...
        $sql = "INSERT INTO floodreport (loc, date_time, report, img, user_id, latitude, longitude, active) VALUES (?, ?, ?, ?, ?, ?, ?, '1')";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssddd", $loc, $formattedDate, $report, $fileData, $user_id, $latitude, $longitude);

        // Execute query
        if ($stmt->execute()) {
            echo '<script>alert("Flood report submitted successfully.")</script>';
            // Query for responders' phone numbers
            $responderQuery = "SELECT contact_no FROM admin WHERE responder_type = 'Flood'";
            ...
            if (mysqli_num_rows($responderResult) == 0) {
                ...
                echo '<script>alert("No flood responders found.")</script>';
                header("Refresh: 1; url=http://urbansafeph.com/User/Guest/report.php");
                exit();
            }

            // Prepare the message content
            $message = "New flood report received at $loc on $formattedDate. Description: $report. Check it: http://urbansafe.ph/User/member.php";
