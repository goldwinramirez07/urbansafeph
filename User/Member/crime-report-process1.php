...
if (!$dateTime || $dateTime->format($dateTimeFormat) !== $date) {
    ...
    if ($timestamp) {
        ...
        header("Refresh: 1; url=http://urbansafeph.com/User/Member/crime-report1.php");
        exit();
    }
} else {
...
if (isset($_FILES['img']) && $_FILES['img']['error'] == UPLOAD_ERR_OK) {
    ...
    if (in_array($fileType, $allowedTypes)) {
        ...
        $sql = "INSERT INTO crimereport (loc, date_time, report, img, user_id, latitude, longitude, active) VALUES (?, ?, ?, ?, ?, ?, ?, '1')";
        ...
        if ($stmt->execute()) {
            ...
            $responderQuery = "SELECT contact_no FROM admin WHERE responder_type = 'Police'";
            ...
            if (mysqli_num_rows($responderResult) == 0) {
                ...
                echo '<script>alert("No police responders found.")</script>';
                header("Refresh: 1; url=http://urbansafeph.com/User/Guest/report.php");
                exit();
            }
            
            // Prepare the message content
            $message = "New crime report received at $loc on $formattedDate. Description: $report. Check it: http://urbansafe.ph/User/member.php";
